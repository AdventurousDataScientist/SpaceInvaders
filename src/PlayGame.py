...
pygame.init()
mixer.music.load('src/Entities/Background/background.wav')
mixer.music.play(-1)
...
bg = pygame.image.load('src/Entities/Background/starter_background.png')
ship = pygame.image.load('src/Entities/Player/player_ship.png')

times = list()
ship_vel = 5

small_missile = pygame.image.load('src/Entities/Player/missiles/large_missile.png')

num_small_enemies = 6

multiple_movement_enemy_image = pygame.image.load('src/Entities/Enemy/enemy_2.png')
enemy_width = 32
enemy_height = 31

enemy_missile = pygame.image.load('src/Entities/Enemy/enemy_missile.png')
...
class Game():
    ...
    def redraw_game_window(self, player_ship, old, cur):
        ...
        draw_text(score, 30, (0, 255, 0), 5, 0)
    ...
    def overlap_check(self, sprite1, sprite2):
        ...
        bottom_in = sprite1.hitbox[1] + sprite1.hitbox[3] > sprite2.hitbox[1] and sprite1.hitbox[1] + sprite1.hitbox[
            3] < sprite2.hitbox[1] + sprite2.hitbox[3]
        left_in = sprite1.hitbox[0] > sprite2.hitbox[0] and sprite1.hitbox[0] < sprite2.hitbox[0] + sprite2.hitbox[2]
        right_in = sprite1.hitbox[0] + sprite1.hitbox[2] > sprite2.hitbox[0] and sprite1.hitbox[0] + sprite1.hitbox[2] < \
                   sprite2.hitbox[0] + sprite2.hitbox[2]
    ...
    def init(self):
        level_layout = open('src/levels.csv')
    ...
    def check_pause(self):
        ...
        keys = pygame.key.get_pressed()
        self.prev_pause = self.cur_pause
        self.cur_pause = keys[pygame.K_p]
        if keys[pygame.K_p]:
            pass
            #print(f'Debugging Dump: Prevous pause: {self.prev_pause} Current_pause: {self.cur_pause}')
        if self.prev_pause and not self.cur_pause:
            if self.pause:
                self.pause = False
            else:
                self.pause = True
    ...
    def set_level(self, player_ship):
        ...
        enemy_type = ''
        num_enemy_type = 0
        enemy = Enemy(0, 0, 32, 31, multiple_movement_enemy_image, 2, 2, 1, 1, 5, random.randint(0, 6), screen_width,
                      screen_height, 0, 0)
        j = 0
            ...
            if int(self.data[self.row][0]) == self.level:
                ...
                while num_enemy_type > 0:
                    if num_enemy_type < n - 1:
                        margin = int(
                            screen_width - (num_enemy_type - 1) * (x_sep) - (num_enemy_type - 1) * (sprite_width)) / 2
                    ...
                    dir = 0
                    image = pygame.image.load('src/Entities/Enemy/enemy_2.png')
                    ...
                    for k in range(0, num_enemies):
                        ...
                        if enemy_type == 'Horizontal_Enemy':
                            score = 7
                            enemy = Multiple_Movement_Enemy(x_loc, y_loc, width, height, image, enemy_x_speed,
                                                            enemy_y_speed, 0, 0, score, shoot, screen_width,
                                                            screen_height, enemy_num_bullets, enemy_health)
                            dir = 0
                            ...
                            score = 10
                            enemy = Erratic_Movement_Enemy(x_loc, y_loc, width, height, image, enemy_x_speed,
                                                           enemy_y_speed, 0, 0, score, shoot, screen_width,
                                                           screen_height, enemy_num_bullets, enemy_health)
                        elif enemy_type == 'Deflector_Enemy':
                            score = 15
                            enemy = Deflector_Enemy(x_loc, y_loc, width, height, image, enemy_x_speed, enemy_y_speed, 0,
                                                    0, score, shoot, screen_width, screen_height, enemy_num_bullets,
                                                    enemy_health)
                        elif enemy_type == 'Boss':
                            score = 30
                            image = pygame.image.load('src/Entities/Enemy/Boss/boss_1.png')
                            width = image.get_width()
                            height = image.get_height()
                            enemy = Boss(x_loc, y_loc, width, height, image, enemy_x_speed, enemy_y_speed, 0, 0, score,
                                         shoot, screen_width, screen_height, enemy_num_bullets, enemy_health)
    ...
    def game_over_screen(self):
        while True:
            ...
            for i in range(0, len(times) - 1):
                ...
                print(f'Time Difference milliseconds: {delta.microseconds / 1000}')
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        pygame.quit()
                        exit()
    ...
    def level_transition(self, player_ship):
        ...
        win.fill((0, 0, 0))
        while x > 0:
            draw_text(score, 30, (0, 255, 0), 55, 0)
            draw_text(health, 30, (255, 0, 0), 351, 0)
            pygame.draw.rect(win, (255, 0, 0), (450, 0, player_ship.health, 15))
    ...
    def play(self, player_ship):
        ...
        while self.running:
            ...
            if not self.pause:
                ...
                boss_old_movement = self.boss_move(boss_current_movement,boss_old_movement)
...
def main():
    ...
    # press 'P' key to pause, it doesnt work properly. Pressing p pauses it, but pressing again, does not unpause it
